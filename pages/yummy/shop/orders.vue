  ...
  <v-app>
    ...
    <v-content>
      <v-container fluid>
        <v-card>
          ...
          <v-data-table ref="table" :headers="headers" :items="orders" :search="search">
            <template slot="items" slot-scope="props">
              ...
              <td class="text-xs-left">
                <v-btn small fab @click="chosenOrder=props.item;showOrderInfo=true;">详情</v-btn>
                <v-btn
                  v-if="props.item.status=='paid'"
                  small
                  fab
                  @click="chosenOrder=props.item;got(props.item);"
                >接单</v-btn>
                <v-btn
                  v-if="props.item.status=='cooking'"
                  small
                  fab
                  @click="chosenOrder=props.item;sent(props.item);"
                >派送</v-btn>
              </td>
            </template>
        ...
        <v-dialog v-model="showOrderInfo" width="500">
          <v-card>
